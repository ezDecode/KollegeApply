...
async function fetchFees() {
	...
	if (title) title.textContent = "Amity University · Fee Structure";
	if (table) {
		table.innerHTML = `
	...
	try {
		const res = await fetch(`${API_BASE_URL}/api/fees/amity`, { cache: "no-store" });
...
function downloadBrochure() {
	...
	link.download = "Amity-Brochure.pdf";
